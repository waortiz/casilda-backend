...
CREATE SEQUENCE IF NOT EXISTS unidadadministrativa_id_seq;
...
SELECT setval('unidadadministrativa_id_seq', (SELECT COALESCE(MAX(id), 0) + 1 FROM unidadadministrativa), false);
...
ALTER TABLE unidadadministrativa ALTER COLUMN id SET DEFAULT nextval('unidadadministrativa_id_seq');
...
ALTER SEQUENCE unidadadministrativa_id_seq OWNED BY unidadadministrativa.id;

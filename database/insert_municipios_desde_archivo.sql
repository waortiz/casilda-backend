-- ============================================================================
-- SCRIPT DE INSERCION DE MUNICIPIOS (DATOS EMBEBIDOS)
-- Sistema CASILDA - FNSP
-- Generado desde ciudades.sql
-- ============================================================================
-- Descripcion:
--   Inserta municipios directamente desde datos embebidos en el script.
--   No requiere archivo externo. El archivo ciudades.sql usa el codigo del departamento.
--
-- Normalizacion aplicada:
--   - Primera letra en mayuscula, resto en minuscula.
--
-- Control de duplicados:
--   - Omite filas cuyo codigo ya exista en la tabla municipio.
--   - Omite filas cuyo nombre (case-insensitive) ya exista en la tabla municipio.
--   - Omite filas cuyo codigodepartamento no exista en la tabla departamento.
-- ============================================================================

BEGIN;

WITH v(codigo, nombre_raw, codigodepartamento) AS (
  VALUES
    ('11001', 'BOGOTA, D.C.', '11'),
    ('13001', 'CARTAGENA', '13'),
    ('13006', 'ACHI', '13'),
...
normalizados AS (
  SELECT
    ...
    UPPER(LEFT(LOWER(TRIM(nombre_raw)), 1)) || SUBSTRING(LOWER(TRIM(nombre_raw)) FROM 2) AS nombre,
    TRIM(codigodepartamento) AS codigodepartamento
  FROM v
  WHERE TRIM(codigo) <> ''
    AND TRIM(nombre_raw) <> ''
    AND TRIM(codigodepartamento) <> ''
),
deduplicados AS (
  SELECT
    codigo,
    nombre,
    codigodepartamento,
    ROW_NUMBER() OVER (PARTITION BY codigo ORDER BY codigo) AS rn_c,
    ROW_NUMBER() OVER (PARTITION BY LOWER(nombre) ORDER BY codigo) AS rn_n
  FROM normalizados
)
INSERT INTO municipio (id, codigo, nombre, iddepartamento)
SELECT
  (SELECT COALESCE(MAX(id), 0) FROM municipio) + ROW_NUMBER() OVER (ORDER BY d.codigo),
  d.codigo,
  d.nombre,
  dep.id
FROM deduplicados d
INNER JOIN departamento dep
  ON dep.codigo ~ '^[0-9]+$'
 AND d.codigodepartamento ~ '^[0-9]+$'
 AND TRIM(dep.codigo)::INT = TRIM(d.codigodepartamento)::INT
LEFT JOIN municipio m
  ON m.codigo = d.codigo
LEFT JOIN municipio mn
  ON LOWER(mn.nombre) = LOWER(d.nombre)
WHERE d.rn_c = 1
  AND d.rn_n = 1
  AND m.id IS NULL
  AND mn.id IS NULL;
...
-- Consultas sugeridas de verificacion:
-- SELECT COUNT(*) FROM municipio;
-- SELECT * FROM municipio ORDER BY id DESC LIMIT 20;